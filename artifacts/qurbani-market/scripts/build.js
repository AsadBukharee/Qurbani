...
async function startMetro(expoPublicDomain, expoPublicReplId) {
  ...
  // Cap Node heap and Metro workers so the production bundle fits within the
  // 4GB Cloud Run build container. Without these, Metro's parallel workers
  // exhaust memory during the Android Hermes minify step and get OOM-killed
  // around 95% — leaving no error message in the build logs.
  const existingNodeOpts = process.env.NODE_OPTIONS || "";
  const env = {
    ...process.env,
    EXPO_PUBLIC_DOMAIN: expoPublicDomain,
    EXPO_PUBLIC_REPL_ID: expoPublicReplId,
    NODE_OPTIONS: /max-old-space-size/.test(existingNodeOpts)
      ? existingNodeOpts
      : `${existingNodeOpts} --max-old-space-size=3072`.trim(),
